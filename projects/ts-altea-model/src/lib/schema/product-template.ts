
import { Branch, DepositMode, DurationMode, DurationReference, Gender, Gift, Invoice, LoyaltyCard, OnlineMode, Order, OrderLine, OrderType, Organisation, Product, ProductOnlineIcons, ProductTypeIcons, Resource, ResourcePlanning, Schedule, Subscription, Task, Template, User, UserBase } from "ts-altea-model";
import { Exclude, Type, Transform } from "class-transformer";
import 'reflect-metadata';
import { ArrayHelper, ConnectTo, DateHelper, DbObjectCreate, IAsDbObject, ManagedObject, ObjectHelper, ObjectMgmt, ObjectReference, ObjectWithId, ObjectWithIdPlus, QueryOperator, TimeHelper } from 'ts-common'
import * as _ from "lodash";
import { PersonLine } from "../person-line";
import { DateRange, DateRangeSet, TimeBlock, TimeBlockSet, TimeSpan } from "../logic";
import * as dateFns from 'date-fns'
import * as Handlebars from "handlebars"
import * as sc from 'stringcase'
import { OrderPersonMgr } from "../order-person-mgr";
import { CancelOrderMessage } from "ts-altea-logic";



export class ProductTemplate extends ObjectWithIdPlus {

    constructor() {
      super()
    }
    
    productId?: string;
  
    @Type(() => Product)
    product?: Product;


    templateId?: string;
  
    @Type(() => Template)
    template?: Template;
  
  }
  